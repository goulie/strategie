...
@section('page_header')
    <div class="container-fluid">
        <div class="row">
            @php
                $directions = App\Models\RhDirection::withCount('services')->get();
            @endphp
...
@section('javascript')
    ...
    <script>
        Highcharts.chart('container', {
                        ...
                        if (!customLabel) {
                            customLabel = chart.options.chart.custom.label =
                                chart.renderer.label(
                                    'Directions Total<br/><strong>{{ $directions->count() }}</strong>'
                                )
                                .css({
                                    color: '#000',
                                    textAnchor: 'middle'
            ...
            title: {
                text: 'Nombre de services par direction'
            },

            tooltip: {
                pointFormat: '<b>{point.y}</b> service(s) — {point.percentage:.0f}%'
            },
            ...
            series: [{
                ...
                data: [
                    @foreach ($directions as $direction)
                        {
                            name: @json($direction->libelle_directions),
                            y: {{ $direction->services_count }}
                        }
                        @if (!$loop->last)
                            ,
                        @endif
